"""
速卖通每日业绩统计 - 定时任务版本
使用 AppleScript 控制现有 Chrome（与手动版相同方案）
"""
...
def get_aliexpress_stats(wait_seconds=15):
    ...
    # 使用 AppleScript 激活现有 Chrome 并打开 URL
    cmd = ['osascript', '-e', 
           'tell application "Google Chrome" to open location "' + target_url + '"']
    subprocess.run(cmd)
